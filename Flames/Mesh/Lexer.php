...
class Lexer
{
    ...
    private int $currentVarBlockLine = 0;
    ...
    public function tokenize(Source $source): TokenStream
    {
        ...
        $this->positionCount = \count($matches[0]);

        while ($this->cursor < $this->end) {
            match ($this->state) {
                self::STATE_DATA => $this->lexData(),
                self::STATE_BLOCK => $this->lexBlock(),
                self::STATE_VAR => $this->lexVar(),
                self::STATE_STRING => $this->lexString(),
                self::STATE_INTERPOLATION => $this->lexInterpolation(),
            };
        }
    ...
    private function pushToken(int $type, int|float|string $value): void
    {
