...
class FilesystemLoader implements LoaderInterface
{
    ...
    public function getSourceContext(string $name): Source
    {
        ...
        // Plain .html files (not .html.twig) are treated as static HTML:
        // Twig syntax is preserved literally by wrapping in {% verbatim %}.
        // .mesh and .html.twig files go through full Twig processing.
        if ($this->isPlainHtml($name)) {
            $code = '{% verbatim %}' . $code . '{% endverbatim %}';
        }

        return new Source($code, $name, $path);
    }

    private function isPlainHtml(string $name): bool
    {
        $lower = strtolower($name);
        return str_ends_with($lower, '.html') && !str_ends_with($lower, '.html.twig');
    }
